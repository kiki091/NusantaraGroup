...
class StaticPage
{
    ...
    protected function setStaticPageCmsTransform($data)
    {
        ...
        $dataTranform['logo_url'] = isset($data['logo_images']) ? asset(STATIC_PAGE_LOGO_IMAGE_DIRECTORY.rawurlencode($data['logo_images'])) : '';
     ...
     * @return array|void
     */
    public function getSingleForEditStaticPageTransform($data)
    {
        if(!is_array($data) || empty($data))
            return array();

        return $this->setSingleForEditStaticPageTransform($data);
    }

    protected function setSingleForEditStaticPageTransform($data)
    {
        $dataTranform['id']  = isset($data['id']) ? $data['id'] : '';
        $dataTranform['site_title'] = isset($data['site_title']) ? $data['site_title'] : '';
        $dataTranform['logo_url'] = isset($data['logo_images']) ? asset(STATIC_PAGE_LOGO_IMAGE_DIRECTORY.rawurlencode($data['logo_images'])) : '';
        $dataTranform['site_name'] = isset($data['site_name']) ? $data['site_name'] : '';
        $dataTranform['favicon_url'] = isset($data['favicon_images']) ? asset(STATIC_PAGE_FAVICON_IMAGE_DIRECTORY.rawurlencode($data['favicon_images'])) : '';
        $dataTranform['og_title'] = isset($data['og_title']) ? $data['og_title'] : '';
        $dataTranform['og_url'] = isset($data['og_images']) ? asset(STATIC_PAGE_OG_IMAGE_DIRECTORY.rawurlencode($data['og_images'])) : '';
