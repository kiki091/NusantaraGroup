<form action="{{ route('NewsFormEditImageSlider') }}" method="POST" id="FormNewsImageSliderForm" enctype="multipart/form-data" @submit.prevent>
					...
					<div class="create__form__row">
						<div class="new__form__field width-auto">
							...
							<div class="form__photo__uploader single__image">
								...
								<div class="form__photo__uploader__wrapper flex flex-align-center">
									<ul class="form__photo__uploader__ul photo-sortable" id="sort_images_slider" v-sort>
										...
										<li class="form__photo__uploader__li sort-item-images-slider" v-for="detailImage in total_detail_image">
											...
											<div class="form__photo__group">
												<div class="form__photo__left">
													<div class="upload__img" v-bind:class="{hide__element: filename_edit[detailImage]}">
														<input name="filename_edit[@{{ $index }}]" class="upload__img__input" type="file" id="filename_edit_@{{$index }}" @change="onImageSliderChange('filename_edit', $index, $event)">
											...
											<a href="javascript:void(0);" class="form__photo__remove__server" v-if="detailImage != 0" @click="removeImageSliderFromServer(images_detail_edit[detailImage].id, detailImage)">&times;</a>
										</li>
								...
								<small><span>Upload Tip: </span>Please upload high resolution photo only with format of *jpeg. <br />(<b>Desktop</b> With Dimension: {{ NEWS_IMAGES_DETAIL_WIDTH }} x {{ NEWS_IMAGES_DETAIL_HEIGHT }} pixels) </small>
							</div>
						</div>
					</div>
				</div>
