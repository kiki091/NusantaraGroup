<form action="{{ route('EventDetailFormEditImageSlider') }}" method="POST" id="FormEventDetailImageSliderForm" enctype="multipart/form-data" @submit.prevent>
		...
		<div class="create__form__wrapper">
			<div class="form--top flex-between">
				...
				<div class="form--top__btn">
					<a href="#" class="btn__add__cancel" @click="resetFormImageSlider">Cancel</a>
				</div>
			</div>
			<div class="form--mid">
										...
										<li class="form__photo__uploader__li sort-item-images-slider" v-for="detailImage in total_detail_image">
											...
											<div class="form__photo__group">
												<div class="form__photo__left">
													<div class="upload__img" v-bind:class="{hide__element: banner_images_edit[detailImage]}">
														<input name="banner_images_edit[@{{ $index }}]" class="upload__img__input" type="file" id="banner_images_edit_@{{$index }}" @change="onImageSliderChange('banner_images_edit', $index, $event)">
														<label for="banner_images_edit_@{{$index }}" class="upload__img__label"></label>
													</div>
													<div class="upload__img" v-bind:class="{hide__element: !banner_images_edit[detailImage]}">
														<img class="upload__img__preview" :src="banner_images_edit[detailImage]" />
														<a href="javascript:void(0);" class="upload__img__show__preview" id="img-preview" @click="previewImage(banner_images_edit[detailImage])">&times;</a>
													</div>
													<span class="form__photo__title">Desktop</span>
												</div>
											</div>
											<a href="javascript:void(0);" class="form__photo__remove__server" @click="removeImageSliderFromServer(banner_edit[detailImage].id, detailImage)">&times;</a>
										</li>
											...
											<div class="form__photo__group">
												<div class="form__photo__left">
													...
													<div class="upload__img" v-bind:class="{hide__element: !banner_images[$index]}">
														...
														<button class="upload__img__remove" @click="removeImageSlider('banner_images', $index)">&times;</button>
													</div>
													<span class="form__photo__title">Desktop</span>
												</div>
											</div>
